    ...
    <div class="container justify-content-center align-items-center text-center">
                    ...
                    <div class="col-auto">
                        @auth
                            ...
                            @if ($spot->likes->where('user_id', Auth::user()->id)->where('spot_id', $spot->id)->count() > 0)
                                {{-- unlike spot --}}
                                <form action="{{ route('spot.like.delete', $spot->id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                            ...
                            @else
                                {{-- like spot --}}
                                <form action="{{ route('spot.like.store', $spot->id) }}" method="post">
                                    @csrf
                                    <button type="submit" class="like-button">
                                        <i class="fa-solid fa-heart"></i>
                                    </button>
        ...
        <div class="row row-cols-1 row-cols-md-4">
            ...
            <div class="col-12 col-md-8 spot-map">
                <h5>Map</h5>
                <div id="map" class="spot-map-container" data-lat="{{ $spot->geo_lat }}"
