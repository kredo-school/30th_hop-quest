            ...
            <div class="col-11 col-md-9">
                <div class="py-4 position-relative w-100">
                    <div class="spot-main-image text-center px-0 rounded-3">
                        <img src="{{ Str::startsWith($cover_image, '/storage') ? asset($cover_image) : asset(ltrim($cover_image, '/')) }}"
                            alt="{{ $title }}" class="h-auto-md-down rounded-3">
                ...
                <div class="row text-center w-100 px-0 mx-0 pt-5">
                    ...
                    <div class="col-md-8 spot-map pe-md-0 ps-md-3 px-sm-0 py-0">
                        ...
                        <div id="map" class="spot-map-container w-100 rounded-3" data-lat="{{ $geo_lat }}"
                            data-lng="{{ $geo_lng }}">
                        </div>
                    </div>
                </div>
        ...
        <div class="row text-center py-3 d-flex justify-content-center">
            <div class="col-5 text-end">
                @if (session('spot_mode') === 'update' && session('spot_id'))
                    <form action="{{ route('spots.edit', ['spot_id' => session('spot_id')]) }}" method="GET"
                        class="w-100">
            ...
            <div class="col-5 text-start">
                @php
                    ...
                    $action = $mode === 'update' ? route('spots.update', session('spot_id')) : route('spots.store');
