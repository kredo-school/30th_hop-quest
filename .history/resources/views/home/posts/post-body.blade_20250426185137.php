    ...
    <div class="card-header border-0 bg-light p-0 overflow-hidden">
        ...
        @if ($post['type'] == 'businesses')
            <a href="{{ route('business.show', $post['id']) }}">
                ...
                @else
                    <img src="{{ asset($post['main_image']) }}" alt="{{ $post['title'] }}" class="post-image">
                @endif
            </a>
        ...
        <div class="row align-items-center personal_space">
            ...
            <div class="col-md-auto col-sm-2 my-auto p-0">
                <button class="btn">
                    @if ($post['avatar'])
                        ...
                        @else
                            <img src="{{ asset($post['avatar']) }}" alt="#" class="rounded-circle avatar-sm">
                        @endif
                    @else
                        <i
            ...
            <div class="col-md-auto col-sm-6 ms-2 p-0">
                <a href="{{ route('profile.header', $post['user_id']) }}"
                    ...
                    <p class="username h4 my-auto" id="username">{{ $post['user_name'] }}</p>
                </a>
            </div>
                ...
                @if ($post['is_liked'])
                    ...
                    <form action="{{ route($likeDeleteRoute, $post['id']) }}" method="post">
                        ...
                        <button type="submit" class="btn p-0">
                            <i
                                class="fa-solid fa-heart home color-red {{ $post['is_liked'] ? 'text-danger' : 'text-secondary' }}"></i>
                        </button>
                    </form>
                @else
                    @php
