<ul class="nav flex-column">
    ...
    <li class="{{ request('tab') == 'followers' ? 'active-item' : '' }}">
        <a href="{{ route('profile.header', ['id' => $user->id, 'section' => 'followers']) }}" class="text-decoration-none"><span class="fw-bold">{{$user->followers->count()}}</span> {{$user->followers->count()==1 ? 'follower' : 'followers'}}</a>
    </li>

    <li class="{{ request('tab') == 'following' ? 'active-item' : '' }}">
        <a href="{{ route('profile.header', ['id' => $user->id, 'section' => 'follows']) }}" class="text-decoration-none"><span class="fw-bold">{{$user->follows->count()}}</span> following</a>
    </li>

    <li class="{{ request('tab') == 'comments' ? 'active-item' : '' }}">
        <a href="{{ route('profile.header', ['id' => $user->id, 'section' => 'comments']) }}" class="text-decoration-none"><span class="fw-bold">{{$user->businessComments->count()+$user->questComments->count()+$user->spotComments->count()}}</span> {{$user->businessComments->count()+$user->questComments->count()+$user->spotComments->count()==1 ? 'comment' : 'comments'}}</a>
    </li>
