...
@php
    ...
    $header = $user->header;
    $headerPath =
        $header && \Illuminate\Support\Str::startsWith($header, ['http', 'data:'])
            ? $header
            : ($header
                ? asset('storage/' . $user->header)
                : null);

    // avatar のパスを決定
    $avatar = $user->avatar;
    $avatarPath =
        $avatar && \Illuminate\Support\Str::startsWith($avatar, ['http', 'data:'])
            ? $avatar
            : ($avatar
...
@section('content')
    ...
    <div class="row">
        <div class="mb-3 pt-3">
            @if ($headerPath)
                <img src="{{ asset('storage/' . $headerPath) }}" class="header-image" alt="Header">
            @else
                <img src="{{ asset('images/logo/header_logo.jpg') }}" class="header-image" alt="Default header">
            @endif
        </div>
    </div>
    {{-- User information --}}
    ...
    <div class="profile-header position-relative">
        <div class="row">
            <div class="col-md-auto col-sm profile-image mb-3">
                @if ($user->avatar)
                    <img src="{{ asset('storage/' . $user->avatar) }}" class="rounded-circle avatar-xxl" alt="Avatar Image">
                @else
                    <img src="{{ asset('images/profiles/avatar.jpg') }}" class="rounded-circle avatar-xxl"
                        alt="Default Avatar">
                @endif
            </div>
            <!-- ... rest of profile content unchanged ... -->
        </div>
    </div>
                                        ...
                                        <div class="col-auto mt-3">
                                            ...
                                            @if ($following->followed->id != Auth::user()->id)
                                                @if ($following->followed->isFollowed())
                                                    {{-- unfollow --}}
                                                    <form action="{{ route('follow.delete', $following->followed->id) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn-following ">Following</button>
                                                ...
                                                @else
                                                    {{-- follow --}}
                                                    <form action="{{ route('follow.store', $following->followed->id) }}"
                                                        method="post">
                                                        @csrf
                                                        <button type="submit" class="btn-follow ">Follow</button>
                                                    </form>
