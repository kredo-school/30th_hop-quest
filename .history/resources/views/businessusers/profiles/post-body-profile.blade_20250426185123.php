...
<div class="card p-3">
    <div class="card-header border-0 bg-light p-0 overflow-hidden">
        ...
        @php
            $imageSrc =
                Str::startsWith($post['main_image'], 'http') || Str::startsWith($post['main_image'], 'data:')
                    ? $post['main_image']
                    : asset(ltrim($post['main_image'], '/'));
        @endphp
    ...
    @if ($section)
        <div class="row align-items-center personal_space">
            ...
            <div class="col-md-auto col-sm-2 my-auto p-0">
                <button class="btn">
                    @if ($post['user']->avatar)
                        <a href="{{ route('profile.header', $post['user_id']) }}"><img
                                src="{{ $post['user']->avatar }}" alt="" class="rounded-circle avatar-sm"></a>
                    @else
                        <a href="{{ route('profile.header', $post['user_id']) }}"><i
            ...
            <div class="col-md-auto col-sm-6 ms-2 p-0">
                <a href="{{ route('profile.header', $post['user_id']) }}"
                    class="text-decoration-none h5 d-inline align-items-center">
                    <p class="h5 my-auto" id="username">{{ $post['user']->name }}</p>
                </a>
            </div>
            ...
            <div class="col-md-auto col-sm-1 mt-1 p-1">
                @if ($post['user']->official_certification == 3)
                    <img src="{{ asset('images/logo/official_personal.png') }}" class="official-personal d-inline ms-0"
    ...
    @if ($post['tab_id'] == 4)
        <div class="row">
            <div class="col p-0">
                @if ($post['duration'])
                    <h5 class="fw-bold"><span>Model Duration: {{ $post['duration'] }}</span>
                        {{ $post['duration'] == 1 ? 'day' : 'days' }}</h5>
                @elseif($post['start_date'] && $post['end_date'])
                    <h5 class="fw-bold">
                        <span>{{ date('M d Y', strtotime($post['start_date'])) }}~{{ date('M d Y', strtotime($post['end_date'])) }}</span>
                    </h5>
                @else
        ...
        <div class="row">
            <div class="col p-0">
                @if ($post['promotion_start'])
                    <h5 class="fw-bold">{{ date('M d Y', strtotime($post['promotion_start'])) }} ~
                        {{ date('M d Y', strtotime($post['promotion_end'])) }}</h5>
                @endif
            </div>
        </div>
    @endif
                ...
                @if ($post['is_liked'])
                    ...
                    <form action="{{ route($likeDeleteRoute, $post['id']) }}" method="post">
                        ...
                        <button type="submit" class="btn p-0">
                            <i
                                class="fa-solid fa-heart color-red {{ $post['is_liked'] ? 'text-danger' : 'text-secondary' }}"></i>
                        </button>
                    </form>
                @else
                    @php
    ...
    <div class="card-footer bg-white">
        ...
        <div class="row">
            ...
            <div class="col-6">
                ...
                @else
                    @if ($post['type'] == 'businesses')
                        <a href="{{ route($postEditRoute, $post['id']) }}"
                    ...
                    @elseif($post['type'] == 'quests')
                        <a
                            href="{{ route('quest.edit', $post['id']) }}"class="btn btn-sm btn-green fw-bold mb-2 w-100">EDIT</a>
                    @elseif($post['type'] == 'spots')
                        <a href="{{ route($postEditRoute, $post['id']) }}"
                            class="btn btn-sm btn-green fw-bold mb-2 w-100">EDIT</a>
                    @endif
