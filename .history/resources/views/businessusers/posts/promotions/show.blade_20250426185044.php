                ...
                <section class="main-image-section">
                    <div class="main-image-wrapper mt-3">
                        @if (Str::startsWith($business_promotion->image, 'http') || Str::startsWith($business_promotion->image, 'data:'))
                            <img src="{{ $business_promotion->image }}" alt="{{ $business_promotions->title }}"
                                class="main-image" alt="Main picture">
                        @else
                            <img src="{{ asset($business_promotion->image) }}" alt="{{ $business_promotion->title }}"
                                class="main-image" alt="Main picture">
                        @endif
                                ...
                                <div class="col-md-auto col-sm-2 my-auto p-0 profile-pic">
                                    <button class="btn">
                                        @if ($business_promotion->user->avatar)
                                            <img src="{{ $business_promotion->user->avatar }}" alt=""
                            ...
                            @if (Auth::user()->role_id == 1)
                                <div class="col-md-1 col-sm-1 ms-auto">
                                    @if ($business_promotion->user->isFollowed())
                                        {{-- unfollow --}}
                                        <form action="{{ route('follow.delete', $business_promotion->user->id) }}"
                                            method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="btn btn-following btn-sm mt-3 w-100">Following</button>
                                        </form>
                                    @else
                                        {{-- follow --}}
                                        <form action="{{ route('follow.store', $business_promotion->user->id) }}"
                                            method="post">
                                            @csrf
                ...
                <div class="row pt-0">
                    <div class="col align-center mb-5">
                        <a
                            ...
                            <button
                                class="btn btn-green text-uppercase w-25 position-absolute start-50 translate-middle mt-1">Back
                                to Profile</button>
                        </a>
                    </div>
                </div>
