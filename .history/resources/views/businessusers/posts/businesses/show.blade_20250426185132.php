            ...
            <section class="main-image-section">
                <div class="main-image-wrapper mt-3">
                    @if (Str::startsWith($business->main_image, 'http') || Str::startsWith($business->main_image, 'data:'))
                        <img src="{{ $business->main_image }}" alt="{{ $business->title }}" class="card-img-top body-image"
                            alt="image">
                    @else
                        <img src="{{ asset($business->main_image) }}" alt="{{ $business->title }}"
                            class="card-img-top body-image" alt="image">
                    @endif
                ...
                <div class="profile-container">
                    <div class="profile-left">
                            ...
                            <div class="col-md-auto col-sm-2 my-auto p-0 profile-pic">
                                <button class="btn">
                                    @if ($business->user->avatar)
                                        <img src="{{ asset('storage' . $business->user->avatar) }}" alt=""
                                            class="rounded-circle avatar-sm">
                                    @else
                                        <i
                                            class="fa-solid fa-circle-user text-secondary profile-sm d-block text-center"></i>
                                    @endif
                                </button>
                            </div>
                        ...
                        <div class="col-md-1 col-sm-1 ">
                            @if (Auth::check())
                                <div class="follow-container" data-user-id="{{ $business->user->id }}"
                                    data-is-followed="{{ $business->user->isFollowed() ? 'true' : 'false' }}">
                                    <button type="button"
                                        class="btn btn-follow btn-sm mt-3 w-100 follow-button {{ $business->user->isFollowed() ? 'd-none' : '' }}">Follow</button>
                                    <button type="button"
                                        class="btn btn-following btn-sm mt-3 w-100 unfollow-button {{ $business->user->isFollowed() ? '' : 'd-none' }}">Following</button>
                                </div>
                            @endif
                        </div>
                                ...
                                @if ($business->isLiked())
                                    <form action="{{ route('businesses.like.delete', $business->id) }}" method="post">
                                        ...
                                        <button type="submit" class="btn p-0">
                                            <i
                                                class="fa-solid fa-heart color-red {{ $business->isLiked() ? '' : 'text-secondary' }}"></i>
                                        </button>
                                    </form>
                                @else
                        ...
                        <tbody>
                            @foreach ($business_hour as $hour)
                                <tr>
                                    ...
                                    <td>{{ isset($hour['break_start']) ? $hour['break_start'] . ' - ' . $hour['break_end'] : 'ー' }}
                                    </td>
                                    <td>{{ $hour['notice'] ?? 'ー' }}</td>
                                </tr>
                            @endforeach
                                    ...
                                    @foreach ($category->businessInfos as $info)
                                        ...
                                        <div class="amenity-item">
                                            ...
                                            <input type="checkbox" id="{{ $info->id }}"
                                                {{ $isValid ? 'checked' : '' }} disabled class="amenity-checkbox" />
                                        </div>
                                    @endforeach
                                </div>
                            </div>
            ...
            <section class="quest-section">
                ...
                <div class="reviews-container">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div>
                                                @if (Auth::check())
                                                    @if ($comment->isLiked())
                                                        <form
                                                            action="{{ route('business.comments.like.delete', $comment->id) }}"
                                                    ...
                                                    @else
                                                        <form
                                                            action="{{ route('business.comments.like.store', $comment->id) }}"
                                                            method="post" class="d-inline">
                                                            @csrf
                                                ...
                                                @else
                                                    <button
                                                        class="btn btn-sm {{ $comment->isLiked() ? 'text-danger' : 'text-secondary' }}"
                                                        disabled>
                                                        <i
                                                            class="fa-{{ $comment->isLiked() ? 'solid' : 'regular' }} fa-heart"></i>
                                                        <span>{{ $comment->BusinessCommentLikes->count() }}</span>
                                                    </button>
                                                @endif
                                            </div>
                    ...
                    <div class="text-center mb-4">
                        <a href="{{ route('business.comments.showcomments', $business->id) }}"
                                ...
                                <form action="{{ route('business.comments.addcomment', $business->id) }}" method="POST">
                                    ...
                                    <div class="mb-3">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="star-rating">
                                                ...
                                                <div class="d-flex">
                                                    <i class="fa-regular fa-star text-warning star-rating-item"
                                                        data-rating="1"></i>
                                                    <i class="fa-regular fa-star text-warning star-rating-item"
                                                        data-rating="2"></i>
                                                    <i class="fa-regular fa-star text-warning star-rating-item"
                                    ...
                                    <div class="mb-3">
                                        <textarea name="content" class="form-control @error('content') is-invalid @enderror" rows="3"
                                            placeholder="Please write your review..." required>{{ old('content') }}</textarea>
                                        @error('content')
                ...
                <div class="top-button-container">
                    <button onclick="window.scrollTo({top: 0, behavior: 'smooth'})" class="top-button">
