...
@section('content')
    ...
    <div class="{{ Auth::user()->role_id === 1 ? 'bg-green' : 'bg-blue' }}">
        <div class="container py-5 col-9">
            ...
            <section id="form2" class="reveal-section">
                <form action="{{ route('questbody.store') }}" method="post" enctype="multipart/form-data" id="body_form"
                    class="bg-white rounded-5 px-5 py-3 my-5">
                    @csrf
                    @method('POST')
                        ...
                        <select id="day_number" name="day_number" class="w-25 p-2 input-box">
                            @foreach ($dayList as $day)
                                <option value="{{ $day['number'] }}"
                                    {{ old('day_number') == $day['number'] ? 'selected' : '' }}>
                    ...
                    @if ($isRole2)
                        <div class="row">
                            ...
                            <div class="col-lg-5 position-relative mt-2">
                                @php
                                    $firstBody =
                                        isset($questBodies) && $questBodies->isNotEmpty()
                                            ? $questBodies->first()
                                            : null;
                                    $isBusinessLocked = $firstBody && $firstBody->business;
                                @endphp

                                <input type="text" name="spot_name" id="spot_name"
                                ...
                                <input type="hidden" name="spot_business_type" id="spot_business_type"
                                    value="{{ old('spot_business_type', $firstBody?->business ? 'business' : 'spot') }}">
                                <input type="hidden" name="spot_business_id" id="spot_business_id"
                                    value="{{ old('spot_business_id', $firstBody?->business->id ?? '') }}">
                                ...
                                <div class="col-lg-6 m-0 p-0">
                                    <p class="xsmall text-secondary m-0">
                                        **You cannot include other businesses in this Quest.
                                        If you want to create a model Quest for a different business, please start a new
                                        Quest.
                                    </p>
                                </div>
                            @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-5 position-relative mt-3">
                                ...
                                <input type="text" name="business_title" id="business_title"
                                    class="input-box form-control w-100 mb-0" value="{{ old('business_title') }}"
                    ...
                    @if ($isRole1)
                        <div class="row">
                            <div class="col-lg-5 position-relative mt-2">
                                <input type="text" name="spot_name" id="spot_name" value="{{ old('spot_name') }}"
                                    placeholder="Tokyo Tower" class="input-box form-control w-100 mb-0">
                                <input type="hidden" name="spot_business_type" id="spot_business_type"
                                    value="{{ old('spot_business_type') }}">
                                <input type="hidden" name="spot_business_id" id="spot_business_id"
                                    value="{{ old('spot_business_id') }}">
                    ...
                    <div class="row mx-0 px-0 text-center">
                        <div class="text-start pb-3 px-0">
                            ...
                            <div class="col-12 px-0">
                                <input type="file" id="image" class="custom-file-input form-control input-box w-100"
                                    multiple>
                                <p id="image-error" class="text-danger small d-none">Please upload at leaset one image.
                                </p>
                            </div>
                            <div class="col-3">
                                <button type="button" class="btn btn-green custom-file-label w-100 me-0 d-none"
                                    id="upload-btn">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-12">
                            <textarea id="introduction" name="introduction" class="text-area input-box w-100" rows="7"
                                placeholder="How was your experience there!">{{ old('introduction') }}</textarea>
                            @error('introduction')
                                <p class="text-danger small">{{ $message }}</p>
                            @else
                    ...
                    <div class="row px-0">
                        <div class="form-check form-switch mx-2">
                            <input type="checkbox" name="agenda" id="agenda" class="form-check-input"
                                {{ old('agenda', '1') ? 'checked' : '' }}>
                            <label for="agenda" class="form-check-label radio-inline raleway-semibold">Add to
                                Agenda</label>
                            <p class="mt-0 xsmall">
                                The Agenda shows a summary of your Quest by listing the place names you’ve selected. You can
                                freely choose whether to display each item in the Agenda, and you can change this later
                                anytime from the Edit page.
                            </p>
                        </div>
                    </div>
            ...
            <section id="header" class="reveal-section">
                <div class="position-relative my-4">
                    @php
                        $hasCertifiedBusiness =
                            $quest->user_id === 2 &&
                            $quest->questBodies->contains(function ($body) {
                    ...
                    @else
                        <img src="{{ asset($quest->main_image) }}" alt="header-img"
                            class="img-fluid w-100 rounded-3 {{ $hasCertifiedBusiness ? 'border-quest-red' : '' }}">
                    @endif

                    @if ($hasCertifiedBusiness)
                        <img src="{{ asset('images/logo/OfficialBadge.png') }}" alt="Certified Badge"
                    ...
                    <div class="overlay position-absolute top-0 end-0 p-3 text-white">
                        ...
                        <div>
                            <button class="btn btn-sm btn-green" data-bs-toggle="modal"
                                data-bs-target="#edit-quest-{{ $quest->id }}">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </button>

                            <button class="btn btn-sm btn-red" data-bs-toggle="modal"
                                data-bs-target="#delete-quest-{{ $quest->id }}">
            ...
            <div id="quest-body-container" class="reveal-section">
                @if (isset($questBodies) && $questBodies->isNotEmpty())
                    ...
                    @foreach ($questBodies->groupBy('day_number') as $day => $bodies)
                        ...
                        @endphp
                        <div class="day-group bg-white rounded-3 p-3 my-5 border {{ $firstBody->border_class }}"
                            data-day="{{ $day }}">
                            <p class="day-number p-4 text-center fs-3 poppins-semibold {{ $firstBody->color_class }}">DAY
                                {{ $day }}</p>
                                ...
                                <div class="spot-entry">
                                    <div class="row pb-3 justify-content-between align-items-center">
                                        <h4 class="spot-name poppins-bold col-md-10 text-start">
                                            ...
                                            @else
                                                @if ($questbody->spot)
                                                    <a href="{{ route('spot.show', ['id' => $questbody->spot->id]) }}"
                                                        class="text-decoration-none text-dark">
                                                        {{ $questbody->spot->title }}
                                                    </a>
                                                @elseif ($questbody->business)
                                                    <a href="{{ route('business.show', ['id' => $questbody->business->id]) }}"
                                                        class="text-decoration-none text-dark">
                                        ...
                                        <div class="col ms-0 text-end pe-0">
                                            <div class="d-flex justify-content-end gap-2 p-0 m-0">
                                                <!-- 編集ボタン -->
                                                <button class="btn btn-sm btn-green edit-modal" data-bs-toggle="modal"
                                                    data-bs-target="#edit-questbody-{{ $questbody->id }}">
                                                    <i class="fa-solid fa-pen-to-square text-white"></i>
                                                </button>

                                                <!-- 削除ボタン -->
                                                <button class="btn btn-sm btn-red delete-questbody-btn"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#delete-questbody-{{ $questbody->id }}">
                                                    <i class="fa-solid fa-trash"></i>
                                                </button>
                                            </div>
                                            <div
                                                class="form-check form-switch d-flex align-items-center justify-content-end mt-2 pe-2">
                                                <input type="checkbox" class="form-check-input agenda-toggle me-2"
                                                    id="agenda-{{ $questbody->id }}" data-id="{{ $questbody->id }}"
                                                    @if ($questbody->is_agenda) checked @endif>
                                                <label class="form-check-label"
                                                    for="agenda-{{ $questbody->id }}">Agenda</label>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-lg-6 image-container d-block flex-column">
                                            @if (!empty($images))
                                                @foreach ($images as $image)
                                                    <img src="{{ asset(ltrim($image, '/')) }}" alt="画像"
    ...
    @if (isset($questBodies) && $questBodies->isNotEmpty())
        @foreach ($questBodies as $questbody)
            @include('quests.modals.quest-body.edit-modal', [
                'questbody' => $questbody,
                'dayList' => $dayList,
            ])
