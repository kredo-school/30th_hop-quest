...
@section('content')
    <div class="{{ Auth::user()->role_id === 1 ? 'bg-green' : 'bg-blue' }}">
        <p
            class="attention color-navy poppins-semibold text-center pt-5 pb-3 fs-4 {{ Auth::user()->role_id == 1 ? 'bg-quest-blue' : 'bg-quest-green' }}">
        ...
        <div class="container py-5 col-9 px-0">
            ...
            <div id="quest-body-container" class="reveal-section revealed">
                ...
                @if (isset($questBodies) && $questBodies->isNotEmpty())
                    @foreach ($questBodies->groupBy('day_number') as $day => $bodies)
                        ...
                        @endphp
                        <div class="day-group bg-white rounded-3 p-3 my-3 border {{ $firstBody->border_class }}"
                            data-day="{{ $day }}">
                            ...
                            @foreach ($bodies as $questbody)
                                <div class="spot-entry">
                                    ...
                                    <div class="row">
                                        @php
                                            $images = is_array(json_decode($questbody->image, true))
                                                ? json_decode($questbody->image, true)
                                                : [];
                                        @endphp
            ...
            <div class="row justify-content-center mt-5">
                <form id="toggle-visibility-form" method="POST"
                    action="{{ route('quest.restore', ['quest_id' => $quest_a->id]) }}"
                    data-quest-id="{{ $quest_a->id }}" class="w-100">
                    @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 d-flex align-items-end justify-content-end pe-4">
                            <a href="{{ route('quest.edit', ['quest_id' => $quest_a->id]) }}"
                                class="btn btn-outline-navy w-75">Back</a>
                        </div>
