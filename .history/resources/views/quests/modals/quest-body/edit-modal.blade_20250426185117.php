...
<div class="modal fade edit-questbody-modal" id="edit-questbody-{{ $questbody->id }}" tabindex="-1"
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content border-green border-4 p-3">
            ...
            <div class="modal-body">
                <form action="{{ route('questbody.update', $questbody->id) }}" method="POST"
                    enctype="multipart/form-data" class="px-3 py-2" id="edit-form-{{ $questbody->id }}">
                    @csrf
                    @method('PUT')
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-md-8">
                            <select name="day_number" id="day_number_{{ $questbody->id }}"
                                class="form-select input-box w-100">
                                @foreach ($dayList as $day)
                                    <option value="{{ $day['number'] }}"
                                        {{ $questbody->day_number == $day['number'] ? 'selected' : '' }}>
                    ...
                    @if (Auth::user()->role_id === 2)
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-md-8">
                                <div class="row mb-3 align-items-center">
                                    <div class="col-md-4">
                                        <label for="business_title_{{ $questbody->id }}" class="form-label">Place
                                            Name</label>
                                    </div>
                                    <div class="col-md-8">
                                        ...
                                        @else
                                            <p id="business-title-error-{{ $questbody->id }}"
                                                class="text-danger small d-none">Please enter a place name</p>
                                        @enderror
                                    </div>
                                </div>
                            </div>
                    ...
                    @endif
                    <div class="mb-3">
                        <label for="introduction_{{ $questbody->id }}" class="form-label"
                            id="introduction">Description</label>
                        ...
                        <p id="intro-error-{{ $questbody->id }}" class="text-danger small d-none">Please enter a
                            description</p>
                    </div>
                    ...
                    <div class="row pb-3 pe-0">
                        ...
                        <div class="col-12">
                            <input type="file" name="images[]" id="edit-image-{{ $questbody->id }}"
                                class="custom-file-input form-control w-100" multiple>
                            <p id="image-error-{{ $questbody->id }}" class="text-danger small d-none">Please upload at
                                least one image</p>
                        ...
                        <div class="ps-3 text-end">
                            ...
                            <div class="d-flex justify-content-end mb-3">
                                <div class="form-check form-switch d-flex flex-row-reverse align-items-center">
                                    ...
                                    <input class="form-check-input" type="checkbox" id="agendaSwitch" name="is_agenda"
                                        value="1" {{ $questbody->is_agenda ? 'checked' : '' }}>
                                </div>
                            </div>
                        </div>
                        {{-- 画像が存在する場合のみ表示 --}}
                        @if (count($existingImages))
                            <div class="existing-image-wrapper row flex-nowrap overflow-auto mt-2"
                                id="image-list-wrapper-{{ $questbody->id }}">
                                @foreach ($existingImages as $img)
                                    <div class="col-auto text-center me-2 position-relative">
                                        <img src="{{ asset(ltrim($img, '/')) }}" alt="画像" class="img-thumbnail"
                                            style="width: 150px; height: auto;">
                                        <button type="button"
                                            class="remove-existing-image btn btn-sm btn-red position-absolute bottom-0 end-0 m-1 text-white"
                                            data-img="{{ $img }}" data-id="{{ $questbody->id }}">
                    ...
                    <div class="text-end">
                        ...
                        <button type="button" class="btn btn-green update-btn"
                            data-id="{{ $questbody->id }}">Update</button>
                    </div>
                </form>
            </div>
        </div>
