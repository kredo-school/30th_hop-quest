    ...
    public function update(Request $request, $id = null)
    {
        ...
        try {
            ...
            if ($request->hasFile('avatar')) {
                ...
                $user->avatar = 'data:image/' . $avatar->extension() . ';base64,' . base64_encode(file_get_contents($avatar));
            }

            if ($request->hasFile('header')) {
                $header = $request->file('header');
                $user->header = 'data:image/' . $header->extension() . ';base64,' . base64_encode(file_get_contents($header));
            }
