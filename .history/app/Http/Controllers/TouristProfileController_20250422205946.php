    ...
    public function update(Request $request, $id = null)
    {
        ...
        try {
            ...
            if ($request->hasFile('avatar')) {
                $avatarPath = $request->file('avatar')->store('images', 'public'); // publicディスクで保存
                $validated['avatar'] = 'storage/' . $avatarPath;
            }

            if ($request->hasFile('header')) {
                $headerPath = $request->file('header')->store('images', 'public');
                $validated['header'] = 'storage/' . $headerPath;
            }
            $user->update($validated);
            \Log::info(":チェックマーク_緑: Profile updated for user ID: {$user->id}");
